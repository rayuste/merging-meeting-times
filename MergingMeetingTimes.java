...
public class Solution {
    ...
    public static List<Meeting> mergeRanges(List<Meeting> meetings) {
        ...
        List<Meeting> copyMeetings = new ArrayList<Meeting>(meetings);
        
        Collections.sort(copyMeetings, new Comparator<Meeting>() {
            public int compare(Meeting m1, Meeting m2) {
                if (m1.getStartTime() > m2.getStartTime()) return 1;
                if (m1.getStartTime() < m2.getStartTime()) return -1;
                return 0;
          }});
        
        List<Meeting> mergedMeetings = new ArrayList<Meeting>();
        Meeting currentMeeting = copyMeetings.get(0);

        for (Meeting meeting : copyMeetings) {
        
            if (meeting.getStartTime() <= currentMeeting.getEndTime()) {
                if (meeting.getEndTime() > currentMeeting.getEndTime()) {
                    currentMeeting.setEndTime(meeting.getEndTime());
                }
            }
